...
/**
 * Format a Date object to RFC 5545 compliant ICS date format in local timezone
 * @param date - The date to format (in UTC)
 * @param timezone - The timezone to use (default: Europe/Brussels)
 * @returns Formatted date string in local timezone (YYYYMMDDTHHMMSS)
 */
function formatICSDateInTimezone(
  date: Date,
  ...
  // Parse the formatted string and convert to ICS format
...
export function generateCalendarInvite(
  ...
  // Build the .ics file content according to RFC 5545 with timezone support
  const icsContent = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    ...
    'BEGIN:VEVENT',
    `UID:${uid}`,
    `DTSTAMP:${formatICSDateInTimezone(createdDate, timezone)}`,
    `DTSTART;TZID=${timezone}:${formatICSDateInTimezone(startDate, timezone)}`,
    `DTEND;TZID=${timezone}:${formatICSDateInTimezone(endDate, timezone)}`,
    `SUMMARY:${summary}`,
